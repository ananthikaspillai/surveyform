...
function Link() {
    ...
    const handleSubmit = async () => {
        try {
            ...
            const questionId = questionsAndOptions[0].id; // Assuming you want to use the first question's ID
            const postdata = { selectedOption };

            const response = await axios.post(
                `http://localhost:8081/surveys/${surveyId}/questions/${questionId}/submit`,
                postdata
            );
